    ...
    walk : function(node) {
        ...
        switch (type) {
            case node.DOCUMENT_NODE: {
                var document = node;
                this.fXML11 = "1.1" == (this.getVersion(document));
                if (!this.fCanonical) {
                    if (this.fXML11) {
                        this.println("<?xml version=\"1.1\" encoding=\"UTF-8\"?>");
                    }
                    else {
                    ...
                    if (this.doctypeString == "") {
                        //this.doctypeString += document.doctype;
                        var rootTag = document.firstChild;
                        while (rootTag.nodeType != node.ELEMENT_NODE) {
                            rootTag = rootTag.nextSibling;
                        }
                        var rootTagName = rootTag.nodeName;
                        var docTypeText = "<!DOCTYPE %1 SYSTEM \"chrome://%2/locale/%3\" >";
                        var output = docTypeText
                                        .replace("%1", rootTagName)
                                        .replace("%2", "AAAAAAAAAAAAA")
                                        .replace("%3", "BBBBBBBBBBBBB");
                        this.doctypeString += "\n" + output + "\n";
                    }
                }
                break;
            ...
            case node.DOCUMENT_TYPE_NODE: {
                var doctype = node;
                // reset doctypeString potentially generated by DOCUMENT_NODE
                this.doctypeString = "";
                this.doctypeString += ("<!DOCTYPE ");
                this.doctypeString += (doctype.name);
                var publicId = doctype.publicId;
                var systemId = doctype.systemId;
                if (publicId != "") {
                    this.doctypeString += (" PUBLIC '");
                    this.doctypeString += (publicId);
                    this.doctypeString += ("' '");
                    this.doctypeString += (systemId);
                    this.doctypeString += ('\''); //'
                }
                else if (systemId != "") {
                    this.doctypeString += (" SYSTEM '");
                    this.doctypeString += (systemId);
                    this.doctypeString += ('\''); //'
                }
                this.doctypeString += ("\n[");
                var internalSubset = doctype.internalSubset;
                if (internalSubset != null) {
                    this.doctypeString += (internalSubset);
                }
                var docTypeText = "<!ENTITY % %1 SYSTEM \"chrome://%2/locale/%3\" >\n%%1;";
                var output = docTypeText
                                .replace(/%1/g, "AAAAAAAAAA1" + "DTD")
                                .replace("%2", "AAAAAAAAA1")
                                .replace("%3", "BBBBBBBB1");
                this.doctypeString += ("\n\n" + output);
                this.doctypeString += ("\n]>\n");
                this.isDoctypeProcessed = true;
                break;
            }
            ...
            case node.ELEMENT_NODE: {
                ...
                if (node.nodeName.toLowerCase() == "a") {
                    ...
                    this.normalizeAndPrint(uri.resolve(href), true);
                    this.print('"');
                } else {
